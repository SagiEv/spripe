...
def rename_sequence(directory, prefix=""):
    ...
    print(f"Successfully renamed {len(temp_files)} files in '{os.path.basename(directory)}'.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Rename an image sequence to a continuous 0000.png, 0001.png... sequence.")
    parser.add_argument("directory", nargs="?", help="The directory containing the PNG sequence")
    parser.add_argument("--all", action="store_true", help="Process all out_python_* directories in the current folder")
    args = parser.parse_args()

    if args.all:
        base_dir = os.path.dirname(os.path.abspath(__file__))
        anim_dirs = [d for d in glob.glob(os.path.join(base_dir, "raw_output", "out_python_*")) if os.path.isdir(d)]
        if not anim_dirs:
