...
def normalize_asset(asset_dir="", width=1920, height=1080, char_height=800, bottom_padding=100, overwrite=False, anim_name=None, progress_callback=None):
    def log(msg):
    ...
    anim_dirs = [d for d in glob.glob(os.path.join(raw_dir, "out_python_*")) if os.path.isdir(d)]
    
    if anim_name:
        anim_dirs = [d for d in anim_dirs if os.path.basename(d).removeprefix("out_python_") in anim_name]
    ...
    for anim_dir in anim_dirs:
        ...
        if os.path.exists(out_dir) and not overwrite:
            log(f"Skipping '{anim_name_str}': '{os.path.basename(out_dir)}' already exists. Use --overwrite to force.")
            continue
        ...
        if first_frame is None or first_frame.shape[2] != 4:
            log(f"Skipping '{anim_name_str}': Invalid image format in 0000.png (must be BGRA).")
            continue
        ...
        for frame_path in frame_paths:
            ...
            if paste_x1 < paste_x2 and paste_y1 < paste_y2:
                ...
                canvas[paste_y1:paste_y2, paste_x1:paste_x2] = resized_frame[src_y1:src_y2, src_x1:src_x2]
...
def main():
    parser = argparse.ArgumentParser(description="Normalize scale and position of sprite animations.")
    parser.add_argument("--width", type=int, default=1920, help="Output canvas width (16:9 ratio, default: 1920)")
    parser.add_argument("--height", type=int, default=1080, help="Output canvas height (16:9 ratio, default: 1080)")
    parser.add_argument("--char-height", type=int, default=800, help="Target character height in pixels (default: 800)")
    parser.add_argument("--bottom-padding", type=int, default=100, help="Padding from the bottom of the canvas (default: 100)")
    parser.add_argument("--asset-dir", type=str, default="", help="Path to the specific asset directory to normalize")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing normalized folders (default: skip)")
    parser.add_argument("--anim-name", type=str, nargs='*', default=[], help="Specific animation(s) to process. If provided, overwrites automatically.")
