...
from spripe.gui.dialogs import (SettingsDialog, NewProjectDialog, NewAssetDialog, 
                         NewAnimationDialog, ExportDialog, 
                         ImportProjectDialog, ImportAssetDialog, DeleteConfirmationDialog)
...
class ActionController:
    ...
    def placeholder_action(self):
        QMessageBox.information(self.mw, "Coming Soon", "This feature is not yet implemented.")
    ...
    def show_new_animation(self, preselected_proj=None, preselected_asset=None):
        ...
        dlg = NewAnimationDialog(projects, self.mw.project_manager.get_assets, proj, asset, self.mw)
        if dlg.exec():
            self.mw.project_manager.create_animation(dlg.selected_project, dlg.selected_asset, dlg.animation_name)
    ...
    def show_import_asset(self, preselected_proj=None):
        ...
        if dlg.exec():
            self.mw.project_manager.create_asset(dlg.selected_project, dlg.asset_name, dlg.asset_path)
            
    def show_import_animation_video(self, preselected_proj=None, preselected_asset=None):
        projects = self.mw.project_manager.get_projects()
        proj = preselected_proj if preselected_proj else self.mw.current_project
        asset = preselected_asset if preselected_asset else self.mw.current_asset
        dlg = NewAnimationDialog(projects, self.mw.project_manager.get_assets, proj, asset, self.mw)
        ...
        if dlg.exec():
            file_path, _ = QFileDialog.getOpenFileName(self.mw, "Select Video", "", "Video Files (*.mp4 *.avi *.mkv)")
            if file_path:
                self.mw.project_manager.create_animation_from_video(dlg.selected_project, dlg.selected_asset, dlg.animation_name, file_path)
    ...
    def show_import_animation_png(self, preselected_proj=None, preselected_asset=None):
        ...
        if dlg.exec():
            dir_path = QFileDialog.getExistingDirectory(self.mw, "Select PNG Sequence Folder")
            if dir_path:
                self.mw.project_manager.create_animation_from_pngs(dlg.selected_project, dlg.selected_asset, dlg.animation_name, dir_path)
            
    def show_export(self, is_animation, preselected_proj=None, preselected_asset=None, preselected_anim=None):
        proj = preselected_proj if preselected_proj else self.mw.current_project
        asset = preselected_asset if preselected_asset else self.mw.current_asset
        anim = preselected_anim if preselected_anim else self.mw.current_animation
        
        if is_animation and not anim:
            QMessageBox.warning(self.mw, "Warning", "Please select an animation to export.")
            return
        ...
        if dlg.exec():
            try:
                self.mw.project_manager.export_item(proj, asset, anim if is_animation else None, dlg.dest_path, dlg.export_type)
    ...
    def show_export_project(self, proj_name):
        ...
        if dlg.exec():
            try:
                self.mw.project_manager.export_item(proj_name, None, None, dlg.dest_path, dlg.export_type)
    ...
    def show_create_folder(self, proj_name):
        text, ok = QInputDialog.getText(self.mw, "Create Virtual Folder", "Folder Name:")
        if ok and text:
    ...
    def show_move_to_folder(self, proj_name, asset_name):
        ...
        item, ok = QInputDialog.getItem(self.mw, "Move to Folder", f"Select destination for '{asset_name}':", folders, 0, True)
        if ok and item:
    ...
    def renormalize_animation(self, proj, asset, anim):
        ...
        if isinstance(anim, list):
            if not anim:
                ...
                self.mw.pipeline_controls.start_worker(normalize_asset, msg, asset_dir=asset_path, overwrite=True)
            else:
                msg = f"Renormalizing {len(anim)} animation(s)..."
                self.mw.pipeline_controls.start_worker(normalize_asset, msg, asset_dir=asset_path, anim_name=anim, overwrite=True)
        else:
