...
class AssetDashboardWidget(QWidget):
    ...
    action_normalize_animation = pyqtSignal(str, str, object) # proj, asset, anim (or list of anims)
    ...
    def __init__(self, project_manager: ProjectManager, parent: Optional[QWidget] = None):
        ...
        sm.metadata_updated.connect(lambda p: self.refresh_view() if p == self.current_project else None)
    ...
    def init_ui(self) -> None:
        ...
        self.anim_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)
        self.anim_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
        self.anim_table.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
    ...
    def refresh_view(self):
        ...
        animations = set(self.project_manager.get_animations(self.current_project, self.current_asset))
        ...
        template_anims = set()
        if virtual_folder:
            template_anims = set(self.project_manager.get_folder_template(self.current_project, virtual_folder))
        ...
        for row, anim in enumerate(all_anims):
            if anim in animations:
                status = self.project_manager.get_animation_status(self.current_project, self.current_asset, anim)
            else:
                status = "Missing"
        ...
        if row >= 0:
            ...
            if status == "Missing":
                ...
                if action == act_normalize:
                    self.action_normalize_animation.emit(self.current_project, self.current_asset, anim)
    ...
    def create_missing_animation(self, anim):
        self.project_manager.create_animation(self.current_project, self.current_asset, anim)
    ...
    def on_normalize_selected(self):
        ...
        if selected_anims:
            self.action_normalize_animation.emit(self.current_project, self.current_asset, selected_anims)
    ...
    def on_normalize_all(self):
        ...
        self.action_normalize_animation.emit(self.current_project, self.current_asset, anims)

    def on_generate_anim(self):
        QMessageBox.information(self, "Coming Soon", "Prompt to Video generator placeholder.")

    def on_set_template(self):
        if not self.current_project or not self.current_asset: return
        ...
        if not folder_options:
            QMessageBox.warning(self, "No Folders", "No virtual folders exist in any project.")
            return
            
        from PyQt6.QtWidgets import QInputDialog
        choice, ok = QInputDialog.getItem(self, "Set as Template", "Select target folder:", folder_options, 0, False)
        
        if ok and choice:
            proj, folder = choice.split(" / ")
            current_anims = self.project_manager.get_animations(self.current_project, self.current_asset)
            ...
            QMessageBox.information(self, "Success", f"Template applied to folder '{folder}'.")
