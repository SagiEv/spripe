...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton, 
                             ...
                             QMessageBox, QApplication, QCheckBox, QGraphicsEllipseItem)
from PyQt6.QtGui import (QPixmap, QPainter, QPen, QColor, QImage, QCursor, QIcon, 
                         QPainterPath, QPolygonF, QRegion, QBitmap)
...
class GrabCutThread(QThread):
    ...
    def run(self):
        ...
        try:
            cv2.grabCut(self.arr, mask, self.rect_cv, bgdModel, fgdModel, 5, cv2.GC_INIT_WITH_RECT)
...
class CanvasView(QGraphicsView):
    ...
    def load_image(self, path):
        ...
        self.selection_overlay = self.scene.addPixmap(QPixmap(self.original_pixmap.size()))
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        if not self.manual_zoom and hasattr(self, 'scene') and self.scene and self.scene.sceneRect().isValid():
            self.fitInView(self.scene.sceneRect(), Qt.AspectRatioMode.KeepAspectRatio)
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if self.current_tool in ['brush', 'eraser']:
                ...
                self.temp_path_item = self.scene.addPath(path, QPen(Qt.GlobalColor.red, 2, Qt.PenStyle.DashLine))
                ...
                self.temp_path_item = self.scene.addRect(rect, QPen(Qt.GlobalColor.red, 2, Qt.PenStyle.DashLine))
                self.temp_path_item.setZValue(20)
    ...
    def draw_on_canvas(self, current_point):
        ...
        if not self.is_soft_brush:
            if self.current_tool == 'brush':
                ...
                pen = QPen(Qt.GlobalColor.transparent, self.pen_size, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap, Qt.PenJoinStyle.RoundJoin)
                painter.setPen(pen)
        ...
        # Soft Brush (Layered Stroke)
            ...
            base_color = self.pen_color if self.current_tool == 'brush' else QColor(0,0,0,255)
            comp_mode = QPainter.CompositionMode.CompositionMode_SourceOver if self.current_tool == 'brush' else QPainter.CompositionMode.CompositionMode_DestinationOut
    ...
    def apply_magic_wand(self, x, y):
        img = self.pixmap_item.pixmap().toImage().convertToFormat(QImage.Format.Format_ARGB32)
        ptr = img.bits()
        ...
        diff = (self.magic_wand_tolerance, self.magic_wand_tolerance, self.magic_wand_tolerance)
    ...
    def apply_grabcut(self):
        ...
        img = self.pixmap_item.pixmap().toImage().convertToFormat(QImage.Format.Format_RGB888)
        ptr = img.bits()
        ptr.setsize(img.sizeInBytes())
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key.Key_Delete or event.key() == Qt.Key.Key_Backspace:
            ...
            if self.drawing and self.current_tool == 'poly_lasso' and len(self.poly_points) > 0:
    ...
    def delete_selection_content(self):
        if not self.pixmap_item or not self.selection_region or self.selection_region.isEmpty(): 
            return
    ...
    def smooth_selection(self, blur_pixels):
        if not hasattr(self, 'original_selection_mask') or self.original_selection_mask is None:
            return
    ...
    def update_selection_overlay(self):
        ...
        mask_qimg = QImage(self._overlay_mask_bytes, w, h, w, QImage.Format.Format_Alpha8)
    ...
    def update_cursor(self, eyedropper_color=None):
        if self.current_tool in ['brush', 'eraser']:
            ...
            transform_scale = self.transform().m11() if self.transform().m11() > 0 else 1.0
            ...
            if self.current_tool == 'brush':
                ...
                if self.is_soft_brush: color.setAlpha(100)
                painter.setBrush(color)
            else:
...
class PainterWidget(QWidget):
    ...
    def init_ui(self):
        ...
        tool_btns = [self.btn_brush, self.btn_eraser, self.btn_eyedropper, self.btn_lasso, self.btn_poly_lasso, self.btn_magic_wand, self.btn_grabcut]
        
        for name, btn in zip(['brush', 'eraser', 'eyedropper', 'lasso', 'poly_lasso', 'magic_wand', 'grabcut'], tool_btns):
        ...
        self.slider_onion.setValue(self.settings_manager.get("onion_opacity_default", 40))
        ...
        self.canvas.grabcut_error.connect(lambda e: QMessageBox.critical(self, "GrabCut Error", f"GrabCut failed:\n{e}"))
    ...
    def update_color_btn(self, color):
        self.btn_color.setStyleSheet(f"background-color: {color.name()}; border: 1px solid #FFF;")
        self.canvas.update_cursor()
    ...
    def save_frame(self):
        ...
        self.btn_save.setStyleSheet("background-color: #28a745; color: white; border: none; border-radius: 4px;")
        
        from PyQt6.QtCore import QTimer
        QTimer.singleShot(1500, lambda: self.reset_save_btn(original_text, original_style))
    ...
    def on_pinned_keyframe_updated(self, path):
        if path and os.path.exists(path):
            ...
            self.canvas.load_pinned_keyframe(path, self.slider_onion.value() / 100.0 if self.chk_onion.isChecked() else 0.0)
        else:
