...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QHBoxLayout, QVBoxLayout, 
                             QWidget, QLabel, QToolBar, QMessageBox, QSplitter, QStackedWidget)
...
class AssetPipelineApp(QMainWindow):
    def __init__(self, base_dir):
        ...
        if getattr(self.settings_manager, 'load_error', None):
            QMessageBox.warning(self, "Settings Error", f"Failed to load settings:\n{self.settings_manager.load_error}\n\nFalling back to defaults.")
        ...
        if getattr(self.project_manager, 'load_error', None):
            QMessageBox.warning(self, "Project Registry Error", f"Failed to load project registry:\n{self.project_manager.load_error}\n\nFalling back to empty registry.")
        ...
        self.timeline_widget.pinned_keyframe_updated.connect(self.painter_widget.on_pinned_keyframe_updated)
        self.painter_widget.interaction_started.connect(self.timeline_widget.pause_anim)
        
        # Connect Context Menu Signals
        self.asset_browser.action_new_asset.connect(self.action_controller.show_new_asset)
        self.asset_browser.action_import_asset.connect(self.action_controller.show_import_asset)
        self.asset_browser.action_export_project.connect(self.action_controller.show_export_project)
        self.asset_browser.action_remove_project.connect(self.action_controller.remove_project)
        self.asset_browser.action_create_folder.connect(self.action_controller.show_create_folder)
        
        self.asset_browser.action_new_animation.connect(self.action_controller.show_new_animation)
        self.asset_browser.action_import_animation_video.connect(self.action_controller.show_import_animation_video)
        self.asset_browser.action_import_animation_png.connect(self.action_controller.show_import_animation_png)
        self.asset_browser.action_export_asset.connect(lambda p, a: self.action_controller.show_export(False, p, a))
        self.asset_browser.action_remove_asset.connect(self.action_controller.remove_asset)
        self.asset_browser.action_move_to_folder.connect(self.action_controller.show_move_to_folder)
        
        self.asset_browser.action_export_animation.connect(lambda p, a, an: self.action_controller.show_export(True, p, a, an))
        self.asset_browser.action_renormalize_animation.connect(self.action_controller.renormalize_animation)
        self.asset_browser.action_remove_animation.connect(self.action_controller.remove_animation)
        
        self.asset_browser.action_new_project.connect(self.action_controller.show_new_project)
        self.asset_browser.action_import_project.connect(self.action_controller.show_import_project)
        ...
        self.project_dashboard.metadata_updated.connect(self.asset_browser.refresh_assets)
        self.project_dashboard.asset_selected.connect(self.on_asset_selected_from_dashboard)
        
        self.asset_dashboard = AssetDashboardWidget(self.project_manager)
        self.asset_dashboard.animation_selected.connect(self.on_animation_selected_from_dashboard)
        self.asset_dashboard.action_new_animation.connect(self.action_controller.show_new_animation)
        self.asset_dashboard.action_normalize_animation.connect(self.action_controller.renormalize_animation)
    ...
    def init_menu_bar(self):
        ...
        new_asset_action.triggered.connect(lambda: self.action_controller.show_new_asset())
        file_menu.addAction(new_asset_action)
        
        new_anim_action = QAction("New Animation", self)
        new_anim_action.triggered.connect(lambda: self.action_controller.show_new_animation())
        ...
        import_asset.triggered.connect(lambda: self.action_controller.show_import_asset())
        import_menu.addAction(import_asset)
        
        import_menu.addSeparator()
        
        import_anim_v = QAction("Import Animation (Video)", self)
        import_anim_v.triggered.connect(lambda: self.action_controller.show_import_animation_video())
        import_menu.addAction(import_anim_v)
        
        import_anim_p = QAction("Import Animation (PNGs)", self)
        import_anim_p.triggered.connect(lambda: self.action_controller.show_import_animation_png())
        ...
        export_asset.triggered.connect(lambda: self.action_controller.show_export(False))
    ...
    def on_item_selected(self, item_type, proj, asset, anim, full_path):
        ...
        if item_type == "project":
            ...
            self.project_dashboard.load_folder(proj, asset) # folder name passed in asset param
            self.right_panel.setCurrentIndex(0)
            ...
            asset_path = ""
            if proj and asset:
                asset_path = os.path.join(self.project_manager.get_project_path(proj), asset)
            elif proj:
