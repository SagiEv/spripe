from PyQt6.QtWidgets import QWidget, QVBoxLayout, QPushButton, QLabel, QListWidget, QHBoxLayout, QInputDialog, QMessageBox, QListWidgetItem, QMenu
...
class ProjectDashboardWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.file_system_view.setSelectionMode(QListWidget.SelectionMode.ExtendedSelection)
        self.file_system_view.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
    ...
    def refresh_view(self, filter_folder=None):
        ...
        if self.current_folder is not None:
            ...
            back_item.setTextAlignment(Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignBottom)
    ...
    def on_edit_template_clicked(self):
        ...
        current_template = self.project_manager.get_folder_template(self.current_project, self.current_folder)
        from spripe.gui.dialogs import TemplateEditorDialog
        dlg = TemplateEditorDialog(current_template, self)
        if dlg.exec():
            self.project_manager.set_folder_template(self.current_project, self.current_folder, dlg.template_lines)
            self.metadata_updated.emit()
            QMessageBox.information(self, "Success", "Folder template updated successfully.")
    ...
    def show_context_menu(self, position):
        ...
        for item in selected_items:
            data = item.data(Qt.ItemDataRole.UserRole)
            if data and data[0] == "back": return
        ...
        has_assets = any(item.data(Qt.ItemDataRole.UserRole)[0] == "asset" for item in selected_items)
        if has_assets:
            menu.addSeparator()
    ...
    def on_rename_item(self, item):
        ...
        new_name, ok = QInputDialog.getText(self, f"Rename {item_type.capitalize()}", "New Name:", text=old_name)
        
        if ok and new_name and new_name != old_name:
            if item_type == "folder":
                success = self.project_manager.rename_virtual_folder(self.current_project, old_name, new_name)
            elif item_type == "asset":
                success = self.project_manager.rename_asset(self.current_project, old_name, new_name)
                
            if success:
                self.metadata_updated.emit()
                self.refresh_view()
            else:
                QMessageBox.warning(self, "Error", f"Could not rename {item_type}. Name may already exist.")

    def on_remove_items(self, items):
        folders = [item.data(Qt.ItemDataRole.UserRole)[1] for item in items if item.data(Qt.ItemDataRole.UserRole)[0] == "folder"]
        assets = [item.data(Qt.ItemDataRole.UserRole)[1] for item in items if item.data(Qt.ItemDataRole.UserRole)[0] == "asset"]
        
        msg = f"Are you sure you want to remove the selected items?\n"
        if folders: msg += f"- {len(folders)} folders\n"
        if assets: msg += f"- {len(assets)} assets\n"
        ...
        if folders:
            folder_reply = QMessageBox.question(self, "Folder Deletion", 
        ...
        for f in folders:
            self.project_manager.delete_virtual_folder(self.current_project, f, delete_assets_in_folders)
    ...
    def on_move_items(self, items):
        ...
        from PyQt6.QtWidgets import QInputDialog
        choice, ok = QInputDialog.getItem(self, "Move to Folder", "Select target folder:", options, 0, False)
        
        if ok:
            vfs = metadata.get("virtual_folders", {})
            for a in assets:
                if choice == "(Root)":
                    if a in vfs: del vfs[a]
                else:
                    vfs[a] = choice
    ...
    def on_items_moved(self, dragged_items, target_item):
        ...
        target_data = target_item.data(Qt.ItemDataRole.UserRole)
        if not target_data or target_data[0] != "folder": return
