...
class AssetBrowser(QWidget):
    ...
    item_selected = pyqtSignal(
        str, str, str, str, str
    )  # item_type, project, asset, anim, full_path
        ...
        for proj in sorted(projects):
            ...
            proj_item.setData(
                ...
                (
                    ...
                    None,
                    None,
                    self.project_manager.get_project_path(proj),
                ),
            )
            ...
            for folder_name in explicit_folders:
                ...
                folder_item.setData(
                    0,
                    Qt.ItemDataRole.UserRole,
                    ("folder", proj, folder_name, None, None),
                )
            ...
            for asset in sorted(assets):
                folder_name = virtual_folders_map.get(asset)
                parent_item = (
                    folder_items.get(folder_name) if folder_name else proj_item
                )
    ...
    def show_context_menu(self, position: QPoint):
        ...
        if item_type == "project":
            ...
            action = menu.exec(self.tree_widget.viewport().mapToGlobal(position))
            if action == act_create_folder:
                self.action_create_folder.emit(proj)
            elif action == act_new_asset:
                self.action_new_asset.emit(proj)
            elif action == act_import_asset:
                self.action_import_asset.emit(proj)
            elif action == act_export_proj:
                self.action_export_project.emit(proj)
            elif action == act_remove_proj:
            ...
            action = menu.exec(self.tree_widget.viewport().mapToGlobal(position))
            if action == act_move_folder:
                self.action_move_to_folder.emit(proj, asset)
            elif action == act_new_anim:
                self.action_new_animation.emit(proj, asset)
            elif action == act_import_anim_v:
                self.action_import_animation_video.emit(proj, asset)
            elif action == act_import_anim_p:
                self.action_import_animation_png.emit(proj, asset)
            elif action == act_export_asset:
                self.action_export_asset.emit(proj, asset)
            elif action == act_remove_asset:
            ...
            action = menu.exec(self.tree_widget.viewport().mapToGlobal(position))
            if action == act_renormalize_anim:
                self.action_renormalize_animation.emit(proj, asset, anim)
            elif action == act_export_anim:
                self.action_export_animation.emit(proj, asset, anim)
            elif action == act_remove_anim:
