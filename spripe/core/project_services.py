...
class WorkspaceRegistry:
    ...
    def load_registry(self) -> None:
        ...
        if self.registry_file.exists():
            try:
                ...
                raise RuntimeError(f"Error loading registry: {e}")
    ...
    def save_registry(self) -> None:
        try:
            ...
            raise RuntimeError(f"Error saving registry: {e}")
...
class ProjectMetadataService:
    ...
    def set_folder_template(self, project_name: str, folder_name: str, template: List[str]) -> None:
        metadata = self.get_metadata(project_name)
...
class FileSystemService:
    def __init__(self, registry: WorkspaceRegistry, metadata_service: ProjectMetadataService):
        self.registry = registry
    ...
    def create_project(self, project_name: str, external_path: Optional[str] = None) -> str:
        if external_path:
    ...
    def create_asset(self, project_name: str, asset_name: str, external_path: Optional[str] = None) -> Tuple[str, str]:
        if not project_name:
        ...
        try:
            ...
            (asset_path / Config.DIR_NORMALIZED_OUTPUT).mkdir(parents=True, exist_ok=True)
    ...
    def create_animation(self, project_name: str, asset_name: str, animation_name: str) -> str:
        if not project_name: project_name = "Standalone"
        if not asset_name: asset_name = "DefaultAsset"
        
        project_path = self.registry.get_project_path(project_name)
        asset_path = project_path / asset_name
        
        target_dir = asset_path / Config.DIR_RAW_OUTPUT / f"{Config.PREFIX_RAW}{animation_name}"
        target_dir.mkdir(parents=True, exist_ok=True)
        SignalManager.get_instance().animation_created.emit(project_name, asset_name, animation_name)
        ...
        if project_name in self.registry.projects:
            if delete_files:
                ...
                try:
                    ...
                    raise Exception(f"Failed to delete project files: {e}")
    ...
    def delete_animation(self, project_name: str, asset_name: str, animation_name: str):
        ...
        targets = [
            ...
            asset_path / Config.DIR_NORMALIZED_OUTPUT / f"{Config.PREFIX_NORMALIZED}{animation_name}"
        ]
        ...
        try:
            ...
            raise Exception(f"Failed to delete animation files: {e}")
            
        SignalManager.get_instance().animation_deleted.emit(project_name, asset_name, animation_name)

    def create_animation_from_video(self, project_name: str, asset_name: str, animation_name: str, video_path: str) -> str:
        if not project_name: project_name = "Standalone"
        if not asset_name: asset_name = "DefaultAsset"
    ...
    def create_animation_from_pngs(self, project_name: str, asset_name: str, animation_name: str, png_dir_path: str) -> str:
        if not project_name: project_name = "Standalone"
        if not asset_name: asset_name = "DefaultAsset"
        ...
        if png_path.name.startswith(Config.PREFIX_NORMALIZED):
            target_dir = asset_path / Config.DIR_NORMALIZED_OUTPUT / f"{Config.PREFIX_NORMALIZED}{animation_name}"
        else:
            target_dir = asset_path / Config.DIR_RAW_OUTPUT / f"{Config.PREFIX_RAW}{animation_name}"
        ...
        try:
            ...
            raise Exception(f"Failed to copy PNG sequence: {e}")
            
        SignalManager.get_instance().animation_created.emit(project_name, asset_name, animation_name)
        return str(target_dir)

    def export_item(self, project_name: str, asset_name: str, animation_name: str, dest_path: str, export_type: str):
        project_path = self.registry.get_project_path(project_name)
        dest = Path(dest_path)
        
        try:
            if animation_name and asset_name:
                src_folder = project_path / asset_name / Config.DIR_NORMALIZED_OUTPUT / f"{Config.PREFIX_NORMALIZED}{animation_name}"
                export_name = f"{asset_name}_{animation_name}"
                
                if not src_folder.exists() or not any(src_folder.iterdir()):
                    raise Exception(f"No normalized frames found for animation '{animation_name}'. Run 'Normalize Animations' first.")
                ...
                norm_output_dir = project_path / asset_name / Config.DIR_NORMALIZED_OUTPUT
                
                if not norm_output_dir.exists() or not any(norm_output_dir.iterdir()):
                    raise Exception(f"No normalized animations found in Asset '{asset_name}'.")
                ...
                if export_type == "ZIP Archive":
                    with tempfile.TemporaryDirectory() as tmpdir:
                        ...
                        for d in norm_output_dir.iterdir():
                            if d.name.startswith(Config.PREFIX_NORMALIZED):
                                shutil.copytree(d, asset_tmp_dir / d.name[len(Config.PREFIX_NORMALIZED):])
                    ...
                    for d in norm_output_dir.iterdir():
                        if d.name.startswith(Config.PREFIX_NORMALIZED):
                            dest_anim_dir = final_dest / d.name[len(Config.PREFIX_NORMALIZED):]
    ...
    def get_animation_status(self, project_name: str, asset_name: str, animation_name: str) -> str:
        asset_path = self.registry.get_project_path(project_name) / asset_name
        
        norm_dir = asset_path / Config.DIR_NORMALIZED_OUTPUT / f"{Config.PREFIX_NORMALIZED}{animation_name}"
        raw_dir = asset_path / Config.DIR_RAW_OUTPUT / f"{Config.PREFIX_RAW}{animation_name}"
