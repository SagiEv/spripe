...
def main():
    ...
    norm_parser.add_argument("--anim", help="Animation name (optional, if omitted processes all animations in asset)", default=None)
    norm_parser.add_argument("--path", help="Absolute or relative path to the asset or animation folder")
    
    # Subcommand: rename
    rename_parser = subparsers.add_parser("rename", help="Re-sequence and rename a folder of PNGs sequentially")
    rename_parser.add_argument("--path", required=True, help="Absolute or relative path to the folder of PNGs")
    
    # Subcommand: reverse
    rev_parser = subparsers.add_parser("reverse", help="Reverse a folder of PNGs")
    rev_parser.add_argument("--path", required=True, help="Absolute or relative path to the folder of PNGs")
...
def run_normalize(args):
    ...
    if args.path:
        ...
        print(f"Normalizing path: {target_path} - (Note: ensure script supports direct path mode)")
