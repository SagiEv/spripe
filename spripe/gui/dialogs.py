...
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel, 
                             ...
                             QComboBox, QMessageBox, QColorDialog, QCheckBox, 
                             QSlider, QListWidget, QListWidgetItem, QWidget, QPlainTextEdit, QStackedWidget)
...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.chk_onion_visible.setChecked(self.settings_manager.get("onion_visible_default", True))
        
        self.slider_onion_opacity = QSlider(Qt.Orientation.Horizontal)
        self.slider_onion_opacity.setRange(0, 100)
        self.slider_onion_opacity.setValue(self.settings_manager.get("onion_opacity_default", 40))
        ...
        self.slider_onion_opacity.valueChanged.connect(lambda v: self.lbl_opacity_val.setText(f"{v}%"))
        ...
        self.chk_boomerang.setChecked(self.settings_manager.get("play_boomerang_default", False))
        self.chk_autoplay = QCheckBox("Autoplay")
        self.chk_autoplay.setChecked(self.settings_manager.get("play_autoplay_default", False))
    ...
    def browse_workspace(self):
        dir_path = QFileDialog.getExistingDirectory(self, "Select Workspace Directory", self.workspace_input.text())
        if dir_path:
            self.workspace_input.setText(dir_path)
            
    def browse_export(self):
        dir_path = QFileDialog.getExistingDirectory(self, "Select Export Directory", self.export_input.text())
        if dir_path:
    ...
    def save_settings(self):
        ...
        self.settings_manager.set("onion_visible_default", self.chk_onion_visible.isChecked())
        self.settings_manager.set("onion_opacity_default", self.slider_onion_opacity.value())
        self.settings_manager.set("play_loop_default", self.chk_loop.isChecked())
        self.settings_manager.set("play_boomerang_default", self.chk_boomerang.isChecked())
        self.settings_manager.set("play_autoplay_default", self.chk_autoplay.isChecked())
        self.accept()
...
class NewAnimationDialog(QDialog):
    def __init__(self, projects, get_assets_callback, current_project, current_asset, parent=None):
        super().__init__(parent)
    ...
    def create_animation(self):
        ...
        if self.asset_combo.count() == 0:
            QMessageBox.warning(self, "Error", "No asset selected. Please create an asset first.")
            return
...
class ExportDialog(QDialog):
    def __init__(self, is_animation, item_name, parent=None):
        ...
        self.item_name = item_name
        self.setWindowTitle(f"Export {'Animation' if is_animation else 'Asset'}: {item_name}")
    ...
    def do_export(self):
        ...
        self.export_type = "ZIP Archive" if "ZIP" in self.type_combo.currentText() else "Folder"
        self.accept()
...
class TemplateEditorDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.list_tags.setStyleSheet("QListWidget::item { background-color: transparent; }")
        ...
        self.input_new_tag.setPlaceholderText("Type a new animation tag and press Enter...")
    ...
    def add_tag_to_list(self, text):
        ...
        btn_del.setStyleSheet("QPushButton { color: white; background-color: transparent; border: none; font-weight: bold; } QPushButton:hover { color: #ff5555; }")
        ...
        if checked:
            ...
            try:
                ...
                if isinstance(data, list):
                    ...
                    QMessageBox.warning(self, "Invalid Format", "JSON must be a list of strings.")
